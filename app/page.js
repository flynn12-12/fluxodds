                  ...
                  <div className={`rounded-xl border p-5 ${arbResult.isArb ? 'bg-emerald-900/5 border-emerald-800/20' : 'bg-[#1a1812] border-[#1e1c16]'}`}>
                    {arbResult.isArb ? (
                      ...
                      <div className="text-center py-4">
                        ...
                        <div className="text-[12px] text-[#5a6a78] mt-2 font-medium">Book hold: <span className="text-[#eef1f5] font-semibold">{arbResult.hold}%</span></div>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div>
                ...
                {holdResult && (
                  <div className={`rounded-xl border p-5 ${holdResult.isArb ? 'bg-emerald-900/5 border-emerald-800/20' : 'bg-[#1a1812] border-[#1e1c16]'}`}>
                    <div className="text-center mb-4">
                      <div className={`text-[36px] font-black leading-none ${holdResult.isArb ? 'text-emerald-400' : parseFloat(holdResult.hold) > 5 ? 'text-red-400' : 'text-[#eef1f5]'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-[#0f0e0b] border border-[#1e1c16] rounded-lg p-3">
                        ...
                        <div className="text-[13px] font-semibold text-[#eef1f5]">Implied: {holdResult.impliedA}%</div>
                      ...
                      <div className="bg-[#0f0e0b] border border-[#1e1c16] rounded-lg p-3">
                        ...
                        <div className="text-[13px] font-semibold text-[#eef1f5]">Implied: {holdResult.impliedB}%</div>
  ...
  const renderMiddleRow = (m, i) => {
    return [
      <div key={m.fingerprint || i}
        ...
        <div>
          <div className={`text-[14px] font-bold ${m.juice <= 0 ? 'text-emerald-400' : 'text-[#eef1f5]'}`}>{m.juice <= 0 ? 'FREE' : `${m.juice}%`}</div>
      ...
      <div key={`m-${m.fingerprint || i}`}
        ...
        <div className="flex items-center gap-4 text-[11px]">
          ...
          <span className={`font-bold ${m.juice <= 0 ? 'text-emerald-400' : 'text-[#eef1f5]'}`}>{m.juice <= 0 ? 'FREE' : `${m.juice}% juice`}</span>
        </div>
      </div>
    ]
  }
                  ...
                  {bonusResults.conversions.map((c, i) => [
                    <div key={`${c.eventID}-${i}`} className="hidden md:grid px-5 py-[14px] border-b border-[#1e1c16] items-center hover:bg-[#0f0e0b] transition-colors" style={{gridTemplateColumns:'1.5fr 1.5fr 1.5fr 100px 100px'}}>
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-[#5a6a78] font-medium mt-[2px]"><span className="text-[#eef1f5] font-semibold">{c.conversionPct}%</span> rate</div>
                      </div>
                    </div>,
                    <div key={`m-${c.eventID}-${i}`} className="md:hidden flex flex-col gap-2 px-4 py-3 border-b border-[#1e1c16] hover:bg-[#0f0e0b] transition-colors">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right flex-shrink-0 ml-3">
                          <div className="text-[18px] font-black text-emerald-400 leading-none">${c.lockedProfit.toFixed(2)}</div>
                          <div className="text-[10px] text-[#5a6a78] font-medium"><span className="text-[#eef1f5] font-semibold">{c.conversionPct}%</span> rate</div>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <div className="flex-1 bg-[#0f0e0b] rounded-lg p-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-11 mt-12 md:mt-16 justify-center">
            {[['40+','Sportsbooks'],['+5.1%','Best live arb'],['<1s','Detection'],['24/7','Always on']].map(([n,l],i) => (
              <div key={i} className="text-center">
                <div className={`text-[28px] md:text-[36px] font-black leading-none ${n.includes('%') ? 'text-emerald-400' : 'text-[#eef1f5]'}`}>{n}</div>
