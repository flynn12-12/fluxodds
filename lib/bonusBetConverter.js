...
async function fetchEventsForLeague(leagueID, apiKey) {
  ...
  try {
    ...
    console.error(`SGO bonus ${leagueID} fetch error:`, e?.message || e);
    return { ok: false, events: [] };
  }
  if (!res.ok) {
    console.error(`SGO bonus ${leagueID} failed: ${res.status} ${res.statusText}`);
    return { ok: false, events: [] };
  }
  let json;
  try {
    ...
    console.error(`SGO bonus ${leagueID} JSON error:`, e?.message || e);
    return { ok: false, events: [] };
  }
  if (!Array.isArray(json?.data)) {
    console.error(`SGO bonus ${leagueID}: unexpected response`);
    return { ok: false, events: [] };
  }
  return {
    ok: true,
...
export async function findBonusConversions(apiKey, { bonusBookID, bonusAmount }) {
  ...
  const fetchComplete = leaguesOk === LEAGUES.length;
  const events = results.flatMap((r) => r.events).filter(isPrematch);
  ...
  if (!fetchComplete && out.conversions.length === 0 && out.scanned === 0) {
    return {
      ...out,
      error:
        'Odds feed is temporarily incomplete — we could not load all leagues. Wait a minute and try again.',
    };
  }
