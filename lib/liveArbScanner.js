...
function buildMarkets(event) {
  ...
  for (const oddID in odds) {
    ...
    for (const bookmakerID in odd.byBookmaker) {
      ...
      const decimal = americanToDecimal(extractAmericanFromBook(book));
      if (!decimal) continue;
      ...
      const marketKey = [
        ...
        normalizePeriodForMarketKey(odd.periodID), odd.betTypeID || '', lineKey,
      ].join('|');
      ...
      const cur = market.sides[sideKey];
