...
// Live arbs are noisier — we keep the realistic-profit cap conservative.
...
// ---------- SGO FETCH ----------
// Note: we don't pass `finalized=false` here because we want both started + non-finalized.
// SGO returns live games when oddsAvailable=true.
...
async function fetchEventsForLeague(leagueID, apiKey) {
  ...
  try {
    ...
    console.error(`SGO ${leagueID} live fetch error:`, e?.message || e);
    return { ok: false, events: [] };
  }
  if (!res.ok) {
    console.error(`SGO ${leagueID} live fetch failed: ${res.status}`);
    return { ok: false, events: [] };
  }
  let json;
  try {
    ...
    console.error(`SGO ${leagueID} live JSON parse error:`, e?.message || e);
    return { ok: false, events: [] };
  }
  if (!Array.isArray(json?.data)) {
    console.error(`SGO ${leagueID} live: unexpected response (expected data array)`);
    return { ok: false, events: [] };
  }
  return {
    ok: true,
...
export async function scanAllLeaguesForLiveArbs(apiKey) {
  const results = await Promise.all(LEAGUES.map((lg) => fetchEventsForLeague(lg, apiKey)));
  const leaguesOk = results.filter((r) => r.ok).length;
  const scanHealthy = leaguesOk === LEAGUES.length;
  const events = results.flatMap((r) => r.events).filter(isLive);
