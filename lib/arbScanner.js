...
import { formatLineKeyForGrouping, twoWayLinesAreConsistent, normalizeStatEntityForMarketKey } from './arbMarketIntegrity';
...
// Soft cap on displayed profit % — catches obvious bad pairings / stale lines while
// still allowing strong legitimate arbs (often ~4–12% on soft vs sharp books).
const MAX_REALISTIC_PROFIT_PCT = 25;
...
function buildMarkets(event) {
  ...
  for (const oddID in odds) {
    ...
    for (const bookmakerID in odd.byBookmaker) {
      ...
      // Group by stat/period/type/line only. Do NOT include oddID — SGO often
      // uses separate odd objects for home vs away moneylines; splitting them
      // prevents two-way arbs from forming.
      ...
      const sideKey = normalizeArbSideKey(odd);
      if (sideKey === 'unknown') continue;

      const current = market.sides[sideKey];
      if (!current || decimal > current.decimal) {
        market.sides[sideKey] = { bookmaker: bookmakerID, american, decimal, line };
      }
    }
  }
...
console.log('[arb-scanner]', JSON.stringify({
    totalEvents: events.length,
    prematchEvents: prematchEvents.length,
    arbsFound: arbs.length,
    leaguesOk,
    sampleEvent: events[0] ? {
      eventID: events[0].eventID,
      leagueID: events[0].leagueID,
      started: events[0].status?.started,
      live: events[0].status?.live,
      oddsCount: events[0].odds ? Object.keys(events[0].odds).length : 0,
    } : null,
  }));
