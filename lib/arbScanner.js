...
async function fetchEventsForLeague(leagueID, apiKey) {
  ...
  try {
    ...
    console.error(`SGO ${leagueID} fetch error:`, e?.message || e);
  ...
  try {
    ...
    console.error(`SGO ${leagueID} JSON parse error:`, e?.message || e);
    return { ok: false, events: [] };
  }

  if (!Array.isArray(json?.data)) {
    console.error(`SGO ${leagueID}: unexpected response (expected data array)`);
    return { ok: false, events: [] };
  }
