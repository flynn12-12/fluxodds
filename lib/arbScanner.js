...
// Sport-aware unit name for totals (e.g. "runs" for MLB, "points" for NBA)
function totalUnit(leagueID) {
  switch (leagueID) {
    case 'MLB': return 'runs';
    case 'NHL': return 'goals';
    case 'NBA': return 'points';
    case 'WNBA': return 'points';
    case 'NFL': return 'points';
    case 'NCAAFB': return 'points';
    case 'NCAAMB': return 'points';
    case 'EPL': return 'goals';
...
function buildSideLabel(event, sideID, betTypeID, line) {
  ...
  if (betTypeID === 'ou') {
    ...
    const unit = totalUnit(event.leagueID);
